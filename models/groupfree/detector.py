...
from models.groupfree.backbone_module import Pointnet2Backbone
from models.groupfree.transformer import TransformerDecoderLayer
from models.groupfree.modules import PointsObjClsModule, FPSModule, GeneralSamplingModule, PositionEmbeddingLearned, PredictHead, \
...
class GroupFreeDetector(nn.Module):
    ...
    def __init__(self,
                 ...
                 size_cls_agnostic=False, dataset_config=None):
        super().__init__()
        ...
        if self.size_cls_agnostic:
            self.proposal_head = ClsAgnosticPredictHead(self.num_class, self.num_heading_bin, num_proposal, 288, dataset_config=dataset_config)
        else:
            self.proposal_head = PredictHead(self.num_class, self.num_heading_bin, self.num_size_cluster,
                                             self.mean_size_arr, num_proposal, 288, dataset_config=dataset_config)
        ...
        for i in range(self.num_decoder_layers):
            if self.size_cls_agnostic:
                self.prediction_heads.append(ClsAgnosticPredictHead(self.num_class, self.num_heading_bin, num_proposal, 288, dataset_config=dataset_config))
            else:
                self.prediction_heads.append(PredictHead(self.num_class, self.num_heading_bin, self.num_size_cluster,
                                                         self.mean_size_arr, num_proposal, 288, dataset_config=dataset_config))
    ...
    def forward(self, end_points):
        ...
        """
        self.eval()
        end_points = self.backbone_net(end_points)
        ...
        if self.sampling == 'fps':
            ...
            end_points['query_points_feature'] = features.permute(0, 2, 1)  # (batch_size, C, num_proposal)
