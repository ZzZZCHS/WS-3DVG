...
from utils.box_util import get_3d_box_torch
...
class PredictHead(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster,
                 mean_size_arr, num_proposal, seed_feat_dim=256, dataset_config=None):
        super().__init__()
    ...
    def forward(self, features, base_xyz, end_points, prefix=''):
        ...
        if prefix == "last_":
            ...
            end_points["size_residuals_normalzied"] = size_residuals_normalized
            end_points["size_residuals"] = size_residuals
            end_points["sem_cls_scores"] = sem_cls_scores
            # end_points["pred_size"] = pred_size

            pred_center = center
            pred_heading_class = torch.argmax(heading_scores, -1)
            pred_heading_residual = torch.gather(heading_residuals, 2, pred_heading_class.unsqueeze(-1)).squeeze(2)
            pred_size_class = torch.argmax(size_scores, -1)
            pred_size_residual = torch.gather(size_residuals, 2, pred_size_class.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, 1, 3)).squeeze(2)
            ...
            pred_bboxs = get_3d_box_torch(pred_obb[:, :, 3:6], pred_obb[:, :, 6], pred_center[:, :, :3])

            end_points["pred_heading"] = torch.zeros_like(pred_heading_class).to(pred_heading_class.device)
            end_points["pred_center"] = pred_center
            end_points["pred_heading_class"] = pred_heading_class
            end_points["pred_heading_residual"] = pred_heading_residual
            end_points["pred_size_class"] = pred_size_class
            end_points["pred_size_residual"] = pred_size_residual
            end_points["pred_bbox_corner"] = pred_bboxs
            end_points["pred_size"] = pred_obb[:, :, 3:6]
...
class ClsAgnosticPredictHead(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_proposal, seed_feat_dim=256, dataset_config=None):
        super().__init__()
    ...
    def forward(self, features, base_xyz, end_points, prefix=''):
        ...
        if prefix == "proposal_":
            ...
            # end_points["pred_size_class"] = pred_size_class
            # end_points["pred_size_residual"] = pred_size_residual
            end_points["pred_bbox_corner"] = bbox
            end_points["pred_size"] = pred_size
